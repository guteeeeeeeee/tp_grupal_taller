...
public class Exception_password_erronea {
	...
	@Test
	public void test_password_erronea() {
		String pass = "cualquiera";
		try {
			Empresa.getInstance().login(this.user1.getNombreUsuario(),pass);
			fail("no lanza que la exception que la password es erronea");
			...
			assertEquals("no coincide con la password ingresada",pass,e.getPasswordPretendida());
			assertEquals("el mensaje no indica que la password es erronea",Mensajes.PASS_ERRONEO.getValor(),e.getMessage());
		}
	}
